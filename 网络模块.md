...
在 `RegisterBuiltInHandlers()` 中向 `ProtocolDispatcher` 注册 HTTP 路由（`GET /ping`、`POST /echo`、`POST /login`、`POST /battle`）
...
accept线程拿到 fd → 入队 → worker轻量握手（构造 ClientContext）→ QueueInLoop → loop线程 AttachConnection
...
- TCP/HTTP 连接一旦进入 EventLoop，通常会占用：连接对象、epoll 关注、心跳、业务状态、内存缓冲等。
...
当前实现中的 `PerformHandshake` 是**轻量握手**，不做阻塞首包读取：

- 构造 `ClientContext`
- 填充 `remoteAddress / remotePort`
- 记录心跳时间 `TouchHeartbeat()`
- 返回成功后进入 `QueueInLoop -> AttachConnection`

也就是说，接入层目前主要负责：接收连接、队列削峰、基础过滤、上下文初始化。
真正的账号/令牌校验在业务路由 `POST /login` 中完成（读取 `x-account` 与 `x-token`）。

#### 3.3 登录失败

在接入层（GuardFilter / 入队上限 / 轻量握手）失败时：
...
连接 attach 成功后才进入 IO 事件循环；业务上的“认证成功”由 `POST /login` 路由决定。
...
- 做连接接入、基础过滤、排队与上下文初始化
- 失败直接关闭连接，保护 EventLoop

2. **业务层（HTTP 路由）**

- `POST /login` 读取 `x-account` / `x-token`
- 调用 `auth::ValidateCredential`
- 通过后设置 `ctx->account` 并 `TransitTo(ClientState::Authed)`，再注册会话

这种方式把“网络接入稳定性”和“业务鉴权策略”解耦，方便后续演进。
...
sequenceDiagram
  ...
  participant D as ProtocolDispatcher
     participant TC as TcpConnection
  C->>LA: TCP 建连
  LA->>LA: GuardFilter + Build ClientContext
  alt 接入层通过
          LA->>EL: QueueInLoop(AttachConnection)
          EL->>TC: 构造 TcpConnection + 注册 epoll
    C->>D: POST /login (x-account/x-token)
    D-->>TC: 200/401
  else 接入层拒绝
    LA-->>C: 关闭连接
     end
