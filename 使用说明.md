...
已内置简单前端页面，包含：登录、游戏主界面、战斗控制（开始/攻击/自动/结束）与战斗日志展示。

生产部署建议：由 Nginx 托管 `src/client` 的静态资源，Nginx 反向代理 API 到 `127.0.0.1:18888`。

`WebGame` 进程本身仅提供 API，不再内置静态文件路由。
...
前端页面由 Nginx 对外暴露（例如 `http://your-domain/`），页面中的接口请求再由 Nginx 转发到 `18888`。

推荐 Nginx 写法（关键是 `proxy_pass` 末尾带 `/`，用于去掉 `/api/` 前缀）：
...
如果使用 `proxy_pass http://127.0.0.1:18888;`（末尾无 `/`），后端会收到 `/api/ping`、`/api/login`、`/api/battle`，与当前服务路由不匹配。
...
```bash
curl -X POST http://127.0.0.1:18888/battle \
	-H 'Content-Type: application/json' \
	-H 'x-account: user1' \
	-d '{"action":"auto","user_team":[2001,2002,2004],"enemy_team":[2005,2006,2003]}'
```

前端武将配置面板依赖以下静态数据文件（建议通过 `src/client/load.sh` 自动同步到 Nginx 目录）：

- `/characters.json`（源文件：`src/data/config/characters.json`）
- `/item_samples.json`（源文件：`src/data/item_samples.json`）

当前默认测试账号（定义于 `src/server/AuthService.cpp`）：
