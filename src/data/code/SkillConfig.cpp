...
bool SkillConfig::loadFromJson(const json& root, std::string* err /*= nullptr*/)
{
	clear();
	if (!root.contains("skills")) {
		LOG_ERROR("SkillConfig: JSON 中缺少 skills 字段。");
		if (err) {
			*err += "SkillConfig: missing skills field";
		}
		return false;
	}
	...
	if (!skillsJson.is_array()) {
		...
		if (err) {
			...
			err->append("SkillConfig: skills is not array");
		}
		return false;
	}
	...
	for (const auto& skillNode : skillsJson) {
		...
		if (skill.id == 0) {
			LOG_ERROR("SkillConfig: 忽略 id=0 的技能记录。");
			ok = false;
			continue;
		}
		...
		skill.trigger = LookupEnum(kSkillTriggerMap, triggerStr, SkillTrigger::None, "SkillTrigger", err);
		...
		if (effectsIt != skillNode.end() && effectsIt->is_array()) {
			for (const auto& eff : *effectsIt) {
				...
				effect.target = LookupEnum(kTargetTypeMap, targetStr, TargetType::SELF, "TargetType", err);

				const std::string effectStr = eff.value("effect", std::string("NONE"));
				effect.effect = LookupEnum(kEffectTypeMap, effectStr, EffectType::NONE, "EffectType", err);
				...
				effect.valueExpr.source = LookupEnum(kValueSourceMap, sourceStr, ValueSource::FIXED, "ValueSource", err);
				effect.valueExpr.owner = LookupEnum(kValueOwnerMap, ownerStr, ValueOwner::CASTER, "ValueOwner", err);
				effect.valueExpr.scale = LookupEnum(kValueScaleMap, scaleStr, ValueScale::ABSOLUTE, "ValueScale", err);
...
bool SkillConfig::loadFromFile(const std::string& path, std::string* err)
{
	std::ifstream in(path);
	if (!in.is_open()) {
		LOG_ERROR("SkillConfig: 无法打开文件 %s", path.c_str());
		if (err) {
			*err += "SkillConfig: failed to open file";
		}
		return false;
	}
	...
	try {
		...
		if (err) {
			if (!err->empty()) err->append("\n");
			err->append("SkillConfig: invalid json");
		}
		return false;
	}
