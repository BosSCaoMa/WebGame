...
## 附录：通用的 GoogleTest 接入步骤
1. **抽离可复用目标**：将业务代码编译成一个（或多个）库目标，主程序只负责链接这些库，方便测试直接复用核心逻辑而无需重复源码清单。
2. **在 CMake 中开启测试**：在根 `CMakeLists.txt` 调用 `include(CTest)` 并用 `if(BUILD_TESTING)` 包裹测试相关配置，默认构建可保持纯净，只有显式传入 `-DBUILD_TESTING=ON` 才生成测试。
3. **准备测试子目录**：在 `test/`（或任意自选目录）单独维护 `CMakeLists.txt`，使用 `FetchContent`、子模块或包管理器拉取 GoogleTest，收集测试源文件并创建测试可执行程序，例如 `add_executable(ProjectTests ...)`，随后 `target_link_libraries(ProjectTests PRIVATE project_core gtest_main)`。
4. **注册到 ctest**：通过 `add_test(NAME ProjectTests COMMAND ProjectTests)` 把可执行文件纳入 `ctest`，这样 `ctest --output-on-failure` 就能统一调度。
5. **编写测试用例**：在测试源文件中 `#include <gtest/gtest.h>`，结合项目头文件使用 `TEST`、`TEST_F`、`SetUp`/`TearDown` 等机制组织断言；若依赖单例或全局状态，务必在每个用例前后重置，确保可重复运行。
6. **独立构建与运行**：推荐为测试使用单独构建目录：
	- `cmake -S . -B build-tests -DBUILD_TESTING=ON`
	- `cmake --build build-tests`
	- `cd build-tests && ctest --output-on-failure`
	如此可同时保留一个仅含主程序的构建目录；若在离线环境，记得将 GoogleTest 依赖指向本地镜像。
